...
module Telegram
  class HTMLParser < BaseParser
    def initialize
      @text = ""
      @entities = [] of API::MessageEntity
      @building_entities = {} of String => API::MessageEntity
    ...
    def parse(text str : String) : Tuple(String, Array(API::MessageEntity))
    ...
    def unparse(text : String, entities : Array(API::MessageEntity), _offset = 0, _length = nil) : String
    ...
    private def handle_start_tag(tag : HTML5::Token)
      ...
      when "a"
        if href = attrs["href"]?
          if match = href.match(/^tg:\/\/user\?id\=(\d+)/)
            ...
            user = API::User.new(match[1].to_i64, false, "")
          else
            url = href
            ent_type = "text_link"
          end
      ...
      if ent_type && !@building_entities.has_key?(tag.data)
        @building_entities[tag.data] = API::MessageEntity.new(
          ent_type,
          offset: @text.size,
